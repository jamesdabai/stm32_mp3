...
int MP3FindSyncWord(unsigned char *buf, int nBytes)
{
	...
	#endif
	for (i = 0; i < nBytes - 1; i++) {
		if ( (buf[i+0] & SYNCWORDH) == SYNCWORDH && (buf[i+1] & SYNCWORDL) == SYNCWORDL )//FF 是每个数据帧的开头
			return i;
	}
...
?*（可能意外地在比特流中找到一个序列
?*似乎匹配，但实际上不是下一个帧头）
?*通过检查几个帧，可以使这个更具有错误恢复能力
?*连续验证nSlots在每种情况下都是相同的
?*由于免费模式需要CBR（见规格），我们通常只会打电话
?*这个函数一次（第一帧）然后存储结果（nSlots）
?*然后从此使用它
?************************************************** **********/
 ...
 *
 * Outputs:     PCM（一种采样技术） data in outbuf, interleaved LRLRLR... if stereo
...
int MP3Decode(HMP3Decoder hMP3Decoder, unsigned char **inbuf, int *bytesLeft, short *outbuf, int useSize)
{
	...
	/* unpack frame header *///解包数据帧的头4字节，FF开头。
	fhBytes = UnpackFrameHeader(mp3DecInfo, *inbuf);////返回帧头数据字节数
	if (fhBytes < 0)	
	{
	    uart_printf("解包的输入数据错误或比特率错误\n");
		return ERR_MP3_INVALID_FRAMEHEADER;		/* don't clear outbuf since we don't know size (failed to parse header) */
	}
	*inbuf += fhBytes;
	
	/* unpack side info */
	siBytes = UnpackSideInfo(mp3DecInfo, *inbuf);//返回边信息所占的字节数//解包边信息
	if (siBytes < 0) {
		MP3ClearBadFrame(mp3DecInfo, outbuf);
		return ERR_MP3_INVALID_SIDEINFO;
	}
	*inbuf += siBytes;//数据向后移动到真实数据
	*bytesLeft -= (fhBytes + siBytes);
	 ...
	 */
	 /* useSize！= 0表示我们正在重新格式化（RTP）数据包（请参阅RFC 3119）//xqy
      * - 调用函数通过移动任何main_data组合“自包含”MP3帧
      *从位存储器（在前面的帧）到同步字和边信息之后
      * - 调用函数应该设置mainDataBegin为0，并告诉我们这是多大
      *帧是（以字节为单位）
     */
	if (useSize) //该值在该程序中为0
	{
		...
		if (mp3DecInfo->mainDataBegin != 0 || mp3DecInfo->nSlots <= 0) 
		{
			...
			/*错误 - 非自包含的帧，或丢失的帧（大小<= 0），可以做丢失隐藏在这里*/
			MP3ClearBadFrame(mp3DecInfo, outbuf);
			return ERR_MP3_INVALID_FRAMEHEADER;
		}
	...
	{
		/* out of data - assume last or truncated frame *///数据不足 - 假定最后一个或截断的帧
		if (mp3DecInfo->nSlots > *bytesLeft) //mp3DecInfo->nSlots该帧数据中真正要转换的数据字节数
		{
		    uart_printf("数据不足\n");
		    //表明该帧的数据不完整，不是在buffer中
			MP3ClearBadFrame(mp3DecInfo, outbuf);
	...
	/* decode one complete frame *///解码一个完整的帧
	for (gr = 0; gr < mp3DecInfo->nGrans; gr++) //不懂
	{
		for (ch = 0; ch < mp3DecInfo->nChans; ch++) //声道数
		{
