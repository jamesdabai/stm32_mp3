...
static QRRawCode *QRraw_new(QRinput *input)
{
    ...
    qr_printf("%s.%d: version=%d, level=%d\r\n", __func__, __LINE__, input->version, input->level);
...
#else
    raw->rsblock = (RSblock *)qr_malloc(raw->blocks * sizeof(RSblock));
    if(raw->rsblock == NULL)
    {
        QRraw_free(raw);
        return NULL;
...
#else
    raw->rsblock = (RSblock *)qr_malloc(1*sizeof(RSblock));
    if(raw->rsblock == NULL)
    {
        MQRraw_free(raw);
        return NULL;
        ...
        memset(raw->rsblock,0, 1*sizeof(RSblock));
#endif
...
static QRcode *QRcode_encodeMask(QRinput *input, int mask)
{
    ...
    qr_printf("%s.%d: version=%d\r\n", __func__, __LINE__, input->version);
    raw = QRraw_new(input);
    if(raw == NULL) return NULL;
...
/**
 *@brief:      QRcode_encodeString
 *@details:   输入字符串生成二维码
 *@param[in]  const char *string  字符串
               int version        二维码版本(参考)
               QRecLevel level    容错等级
               QRencodeMode hint  编码类型
               int casesensitive  是否进行大小写转换 1否 0是 默认否
 *@param[out]  无
 *@retval:     QRcode
 */
